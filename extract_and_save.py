...
python -m dinov3_feat.extract_and_save \
    ...
    --desc_layer 22 26 30  <-- 支持多个层号

说明：
- save_dir 为相对 HOME 的路径，最终保存到：~/<save_dir>/<model_name>/<layer>/... 文件。
"""
...
def main():
    ...
    parser.add_argument('--dinov3_local_path', default='/root/autodl-tmp/dinov3-main',type=str, help='DINOv3 本地 torch.hub 仓库路径')
    ...
    parser.add_argument('--save_dir', type=str, default='/root/autodl-tmp/0-pipei-dinov3/feats', help='相对 HOME 的保存目录名')
    
    # 修改：支持多个层号输入，nargs='+'
    parser.add_argument('--desc_layer', type=int, nargs='+', default=None, help='中间层编号列表（例如 22 26 30）；不提供则从 YAML 读取或默认为最后一层）')
    ...
    def pick(val_cli, val_yaml, default=None):
        # 注意 val_cli 现在可能是一个列表（对于 desc_layer），不能简单用 not in [None, ''] 判断空列表
    ...
    print(f"[配置] model_name={model_name}, agg={agg_name}, num_c={num_c}, vlad_cache_dir={vlad_cache_dir}, desc_layers={desc_layers}, desc_facet={desc_facet}, use_cls={use_cls}")
    ...
    extractor = Dinov3Extractor(
        ...
        desc_layer=desc_layers, # 传入列表
        desc_facet=desc_facet,
        use_cls=use_cls,
    )

    # 提取并保存
    # 现在 extract_loader 返回的是 {layer_idx: feat_tensor} 的字典
    print(f'==> 提取 gallery 特征 (Layers: {extractor.desc_layers})...')
    gall_feats_dict, gid, gall_name = extractor.extract_loader(gall_loader)
    
    print(f'==> 提取 query 特征 (Layers: {extractor.desc_layers})...')
